...
library(remotes)
library(BiocManager)
remotes::install_local("fgsea_1.37.4.tar.gz")
remotes::install_local("systemfonts_0.3.2.tar.gz")
install.packages("Rcpp")
remotes::install_local("RcppArmadillo_0.10.8.1.0.tar.gz")
BiocManager::install("tidytree")
remotes::install_local("treeio_1.10.0.tar.gz")
remotes::install_local("GOSemSim_2.10.0.tar.gz")
remotes::install_local("DOSE_3.12.0.tar.gz")
remotes::install_local("ggplotify_0.1.0.tar.gz")
remotes::install_local("enrichplot_1.6.0.tar.gz")
remotes::install_local("org.Mm.eg.db_3.10.0.tar.gz")
remotes::install_local("rvcheck_0.1.6.tar.gz")
remotes::install_local("clusterProfiler_3.12.0.tar.gz")
...
library(DEGseq)
library(data.table)
geneExpFile <-data.frame(fread("/data/gene_count_matrix.csv",header=T))
...
DEGexp(geneExpMatrix2 = control_positive_df, geneCol1 = 1, expCol1 = 2:4, depth1 = control_positive_depth, groupLabel1 = "Tre",
       ...
       method = "LRT", normalMethod = "median", outputDir = "/results")
...
contr_compare<-data.frame(fread("/data/control+G vs -G adj.csv",header=T))
Tfus_compare<-data.frame(fread("/data/tfus+G vs -G adj.csv",header=T))
Tfus_contr_compare<-data.frame(fread("/data/tfus+G vs control +G adj.csv",header=T))
M<-contr_compare
M<-Tfus_compare
...
ggsave('/results/vocalno.pdf',plot = p1,width = 8,height = 6)
...
write.csv(contr_compare,file="/results/contr_compare.csv")
write.csv(Tfus_compare,file="/results/Tfus_compare.csv")
write.csv(Tfus_contr_compare,file="/results/Tfus_contr_compare.csv")

############Venn painting
library(grid)
library(vctrs)
library(ggvenn)
conUp<-data.frame(fread("/data/conUp.csv",header=T))
tfusUp<-data.frame(fread("/data/tfusUp.csv",header=T))
tvcUp<-data.frame(fread("/data/tvcUp.csv",header=T))
...
ggsave('/results/venn_Up.pdf',plot = Up,width = 8,height = 6)

conDown<-data.frame(fread("/data/conDown.csv",header=T))
tfusDown<-data.frame(fread("/data/tfusDown.csv",header=T))
tvcDown<-data.frame(fread("/data/tvcDown.csv",header=T))
...
Down<-ggvenn(x,
             ...
             fill_color = c("#b2e7cb","#b2d4ec","#ffb2b2"),
             set_name_color = c("#4a9b83","#1d6295","#ff0000"))
ggsave('/results/venn_Down.pdf',plot = Down,width = 8,height = 6)
...
go_enrich_results_ALL <- enrichGO(gene = gene_ENTREZID,
                                  ...
                                  qvalueCutoff  = 0.05,
                                  readable      = TRUE)
write.csv(go_enrich_results_ALL@result, '/results/GO_gene_ALL_enrichresults.csv') 
...
bp <-data.frame(fread("/data/GO_gene_ALL_enrichresults_tfusplustvcBP.csv",header = T))
mf <-data.frame(fread("/data/GO_gene_ALL_enrichresults_tfusplustvcMF.csv",header = T))
cc <-data.frame(fread("/data/GO_gene_ALL_enrichresults_tfusplustvcCC.csv",header = T))
...
ggsave('/results/GO_BP.pdf',plot = p,width = 10,height = 6)

#########KEGG terms painting
kegg_up <-data.frame(fread("/data/KEGG_gene_enrichresults_tfusplustvc.csv",header = T))
kegg_down <-data.frame(fread("/data/KEGG_gene_enrichresults_tfusdowntvc.csv",header = T))
d<-kegg_up
d<-d[order(-d$Count),]
d <-d[1:10,]
d$Description <- factor(d$Description,levels=d$Description)
...
ggsave('/results/kegg_all.pdf',plot = p,width = 10,height = 8)

################preparation file for cytoscape and Upset of GO terms
library(UpSetR)

go_results<- data.frame(fread("/data/pathway_select.csv",header = T))
...
write.table(nodes, "/results/nodes.txt", sep = "\t", row.names = FALSE)
write.table(edges, "/results/edges.txt", sep = "\t", row.names = FALSE)
...
p<-upset(fromList(x), 
         ...
         shade.color = "gray88") 
pdf("/results/upset_plot.pdf", width = 10, height = 8)
print(p)
dev.off()
...
library(ggplot2)
library(reshape2)
select <- data.frame(fread("/data/filtered_expr.csv",header = T))
...
ggsave('/results/heat_map_select.pdf',plot = p,width = 12,height = 3)